...
function refreshMessages() {
  ...
  const controls = m
    ? `<p role="status" data-message-status>${esc(status)}</p><div class="row">${control('force-open', 'Force open', !m.available)}${control('close', 'Close on TV', !m.open)}${control('dismiss', 'Dismiss')}</div>${!m.available ? '<p class="hint">Show this player in TV & layout and show the TV overlay before opening their message.</p>' : ''}<div class="row space-top">${control('previous', '← Previous page', !m.bodyVisible || m.page === 0)}<span>Page ${m.page + 1} of ${m.pageCount}</span>${control('next', 'Next page →', !m.bodyVisible || m.page + 1 === m.pageCount)}</div><p class="hint">Reading controls work while the TV is click-through. “Opened” means displayed on the TV, not proof that the player read it.</p><div class="space-top">${control('review', 'View sent text on laptop')}</div><div class="message-sent-text" data-message-review></div>`
