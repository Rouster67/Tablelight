...
- Adds uploaded ability icons to shared and character-only entries. Upload, replace or remove PNG/JPEG/static WebP artwork; fitted thumbnails appear on both screens and travel with backups.
- Sends a message to one selected player's overlay. An unread envelope shows no preview; players can open it, and the DM can force open, page, scroll, close or dismiss it even in click-through mode. Opened text is visible on the shared TV; messages clear when the app closes or a backup is restored.
